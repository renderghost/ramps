...
A single-page tool for designing SVG radial gradients and exporting them as print-ready SVG or PNG files. You pick a set of colors, shape the gradient with a handful of sliders, and download the result.
...
- **Colors and the color count** are saved to `localStorage` (`color-0`, `color-1`, …) and survive a page reload.
...
| Control              | Element(s)      | Does                                                                                |
| --------------------- | --------------- | ------------------------------------------------------------------------------------ |
| **Download as SVG**   | `#download-svg` | Serializes the live `<svg>` and downloads it as `gradient.svg`.                     |
| **Download as PNG**   | `#download-png` | Rasterizes the same SVG onto a 2000×2000 canvas and downloads it as `gradient.png`. |

These two buttons sit at the top of the panel, directly under the "Ramps" heading.
...
- "Randomise Sliders" randomizes everything at once; there's no per-control randomize or "lock" to explore variations of one axis at a time.
